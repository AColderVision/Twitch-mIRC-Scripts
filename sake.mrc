...
ON $*:TEXT:/^!sake(\s|$)/iS:%mychan: {
  ...
  ELSE {
    ...
    ELSEIF ($regex($calc($readini(sake.ini,_STATS_,total) / %sake_trigger),^\d+$)) {
      MSG $chan acvSAKE $nick $+ -san has poured A Colder Vision the SUPER MEGA SAKÉ SHOT! To date, A Colder Vision has consumed $bytes($readini(sake.ini,_STATS_,total),b) cups of Sakè! acvSAKE %msg
      VAR %sound_file Sake_1000.mp3
    }
    ELSE {
...
ON $*:TEXT:/^!smstrigger\s\d+$/:%mychan: {
  IF ($isEditor) {
    SET %sake_trigger $2
    MSG $chan acvSAKE The Super Mega Sakè is now triggered every %sake_trigger pours of Sakè! acvSAKE
  }
}
